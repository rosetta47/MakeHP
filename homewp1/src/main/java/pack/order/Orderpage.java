...
//	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//세션을 만들면 알아서 쿠키를 양쪽 서버에다가 알아서 만든
				...
				ArrayList<ProductDto> glist = (ArrayList<ProductDto>)session.getAttribute("list");//장바구니할때 이걸 많이써 arraylist
				// 냉장고에서 list를 꺼내서 glist를 줌(냉장고 안에서 밖으로 뺌)
				if(glist == null) glist = new ArrayList<ProductDto>(); // Goods 객체를 담을 glist 생성
				//glist 는 null이니까 glist 컬렉션을 만들어(냉장고 밖에서 일어나는 일)
				glist.add(new ProductDto(name,price));
